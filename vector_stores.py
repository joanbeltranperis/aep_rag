...
def create_vector_stores(config: RagConfig) -> tuple[FAISS, FAISS]:
    ...
    print(colored(f"Successfully parsed {successful_chapters}/{config.total_chapters} chapters", "green"))
    ...
    print(colored(f"Original vector store saved to: {config.vector_store_path}", "green"))
    ...
    print(colored(f"Split {len(documents)} documents into {total_chunks} chunks", "green"))
    ...
    print(colored(f"Split vector store saved to: {config.split_vector_store_path}", "green"))
...
def parse_document_from_url(url: str) -> list[Document]:
    ...
    try:
        ...
        chapter_title = chapter_title_element.get_text(strip=True) if chapter_title_element else "Unknown Chapter"
...
def parse_tables(url: str) -> list[Document]:
    """Parse tables from a URL (currently not used but kept for future extension)."""
    try:
